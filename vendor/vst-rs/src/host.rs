...
impl PluginInstance {
    ...
    pub fn get_parameter_name(&self, i: i32) -> String {
        self.read_string_param(plugin::OpCode::GetParameterName, i, 0, 0.0, 2000)
    }

    pub fn initial_delay(&mut self) -> i32 {
        let effect = self.params.borrow_mut().effect.get();
        (unsafe { &**effect }).initialDelay
    }
}
